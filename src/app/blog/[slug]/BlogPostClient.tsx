    ...
    <div>
      {blocks.map((block, i) => {
        ...
        if (trimmed.startsWith('## ')) {
          ...
          return (
            <h2
              key={i}
              id={id}
              className="font-display text-2xl sm:text-3xl font-bold text-espresso mt-12 mb-6 scroll-mt-24"
            >
        ...
        if (trimmed.startsWith('### ')) {
          ...
          return (
            <h3
              key={i}
              id={id}
              className="font-display text-xl sm:text-2xl font-semibold text-espresso mt-10 mb-4 scroll-mt-24"
            >
              <InlineRenderer text={text} />
            </h3>
          )
                  ...
                  <thead>
                    <tr className="border-b-2 border-espresso/20">
                      {headerCells.map((cell, ci) => (
                        <th
                          key={ci}
                          className="text-left py-3 px-4 font-semibold text-espresso"
                        >
                      ...
                      return (
                        <tr key={ri} className="border-b border-deep-cream">
                          {cells.map((cell, ci) => (
                            <td
                              key={ci}
                              className="py-3 px-4 text-muted-brown"
                            >
                              <InlineRenderer text={cell} />
                            </td>
                          ))}
          ...
          return (
            <ul key={i} className="my-6 space-y-3">
              {items.map((item, li) => (
                <li
                  key={li}
                  className="flex items-start text-muted-brown leading-relaxed"
                >
                  <span className="text-amber mr-3 mt-1.5 text-xs flex-shrink-0">
                    &#9679;
                  </span>
                  <span>
                    <InlineRenderer text={item.replace(/^[-*]\s+/, '')} />
                  </span>
                </li>
              ))}
            </ul>
            ...
            <ol key={i} className="my-6 space-y-3">
              {items.map((item, li) => (
                <li
                  ...
                  className="flex items-start text-muted-brown leading-relaxed"
                >
                  <span className="text-amber font-semibold mr-3 flex-shrink-0 min-w-[1.5rem]">
                    {li + 1}.
                  </span>
                  <span>
                    <InlineRenderer text={item.replace(/^\d+\.\s+/, '')} />
                  </span>
                </li>
              ))}
            </ol>
        ...
        return (
          <p
            key={i}
            className="text-muted-brown leading-relaxed text-base sm:text-lg my-5"
          >
  ...
  return (
    <div className="my-12 bg-espresso p-8 sm:p-10 text-center" style={{ borderRadius: '4px' }}>
      ...
      <p className="text-cream/70 mb-6 max-w-lg mx-auto">
        Book a free feasibility call with our team. We will give you an honest
        assessment of whether franchising is right for your business.
      </p>
      <Link
        href="/contact"
...
export default function BlogPostClient({
  ...
  // Split content roughly in half to insert mid-article CTA
    ...
    <>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16 sm:py-20">
          ...
          <nav className="mb-8" aria-label="Breadcrumb">
            <ol className="flex items-center gap-2 text-sm text-cream/50">
              ...
              <li>/</li>
              <li className="text-cream/70 truncate max-w-[200px] sm:max-w-none">
                {post.title}
              </li>
            </ol>
          </nav>
      ...
      <section className="bg-espresso text-cream">
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-16 sm:py-20 text-center">
          ...
          <p className="text-lg sm:text-xl leading-relaxed text-cream/70 mb-10 max-w-2xl mx-auto">
            If this article raised questions about franchising your business, we
            are here to answer them. Book a free feasibility call and get a
            straight answer.
          </p>
      ...
      {relatedPosts.length > 0 && (
        <section className="bg-deep-cream">
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16 sm:py-20">
            <h2 className="font-display text-2xl sm:text-3xl font-bold tracking-tight text-espresso mb-10">
              Related Articles
            </h2>
